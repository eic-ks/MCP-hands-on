...
## Tool Schema
- 自作したtoolのtool schemaをTOOLとしてAPIリクエストに含める
- Claude Codeのような既成エージェントはAPIレベルで送信内容を変えられないので、Tool Schemaを含めることができず、自作Toolを使わせることはできない。（Anthropic APIのようなAPIレベルなら可能）
- MCPサーバーを立てて接続することはできるので、MCPサーバーを通じて自作Toolの公開が可能
...
- 同じユーザー・同じブラウザセッションでは値が保持される。
**ページの再実行（ボタン、入力変更など）**でも消えないが、ブラウザを閉じる・セッションが終了する・アプリを再起動すると基本的にリセットされる。
- 他のユーザーとは共有されず、各ユーザーごとに独立した状態を持つ。
